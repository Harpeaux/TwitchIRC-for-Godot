...
# TwitchIRC 1.1 (3.2+) logs into the provided channel's IRC and broadcasts incoming chat messages in realtime via the chat_message signal
...
var usernotice_regex:RegEx = RegEx.new()

var connection_timer:Timer = null

signal connection_timeout
signal channel_not_found
signal channel_joined

# setup
func _ready()->void:
	chat_regex.compile("PRIVMSG #[^ ]+ :(.+)")
	usernotice_regex.compile("^@([^ ]+) USERNOTICE #([^ ]+)")
	user_regex.compile("^:(\\w+)!")

# start initial connection to twitch server
...
func _handle_response(response:String) -> void:
	...
	for line in lines:
		...
		# can't find provided channel
		if line.find(" 403 ") != -1 or line.find("No such nick/channel") != -1:
			var parts = line.split(" ")
			if parts.size() >= 4:
				var bad_channel = parts[3].replace("#","")
				if bad_channel.to_lower() == channel.to_lower():
					if dbp:print("Channel does not exist:", bad_channel)
					emit_signal("channel_not_found", bad_channel)
					logout()
					return
		...
		if not joined_channel and line.find(" 001 ") != -1:
			...
			connection_timer.start(3.0)
			yield(connection_timer, "timeout")
			connection_timer.queue_free()
			connection_timer = null
			
			# If still not joined after timeout, logout
			if not joined_channel:
				if dbp:print("Channel join timeout. Channel may not exist.")
				emit_signal("connection_timeout", channel)
				logout()
			continue
		
		# successfully joined channel
		if line.find(" 366 ") != -1:
			var parts_join:PoolStringArray = line.split(" ")
			if parts_join.size() >= 4:
				var joined: String = parts_join[3].replace("#", "")
				if joined.to_lower() == channel.to_lower():
					joined_channel = true
					oauth_login = not OAUTH.empty()
					if dbp:print("Successfully joined: ", joined)
					emit_signal("channel_joined", joined)
			continue
...
func build_privmsg(username:String,body:String,bits:int = 0)->String:
	...
	tags += "tmi-sent-ts=%d;" % (OS.get_unix_time() * 1000)
...
func build_usernotice(username:String,type:int,extra:Dictionary = {})->String:
	...
	var timestamp:String = str(OS.get_unix_time() * 1000)
